/// Task protocol — rules and enforcement for the task-partitioned
/// conversation memory system.
///
/// The canonical task types (TaskStatus, Task) live in eddie_shared/task.
/// This module defines the protocol checking logic and the rules text
/// shown to the LLM as a system prompt.
...
import eddie_shared/task.{type Task, type TaskStatus}
...
fn check_close(
  ...
  case active_task_id {
    ...
    Some(tid) -> {
      let has_memories =
        dict.get(tasks, tid)
        |> result.map(fn(task_item) { task_item.memories != [] })
        |> result.unwrap(False)
      case has_memories {
        True -> None
        False ->
